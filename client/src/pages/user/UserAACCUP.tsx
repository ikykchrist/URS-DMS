...
export default function UserAACCUP() {
  ...
  const loadAreas = useCallback(async () => {
    ...
    setError(null)
    try {
      const nextAreas = await listOnlineAaccupAreas()
      setAreas(nextAreas)
      setActiveAreaId((current) => current || nextAreas[0]?.id || "")
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "Unable to load dynamic AACCUP areas")
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader
        ...
        description="Live requirements configured by the university Requirement Builder."
      ...
      <Card className="mt-5 overflow-hidden border-slate-200/70">
        <CardContent className="p-3">
          {areasLoading ? <Skeleton variant="rectangular" className="h-11" />
            : areas.length === 0 ? <p className="py-3 text-center text-[12px] text-slate-500">No active AACCUP areas are assigned to your account.</p>
