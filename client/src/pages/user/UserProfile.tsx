...
const getRoleBadge = (role: string) => {
  switch (role) {
    case "super_admin": return <Badge variant="default">Administrator</Badge>
    case "faculty": return <Badge variant="secondary">Faculty</Badge>
    case "department_head": return <Badge variant="secondary">Department Head</Badge>
    case "qa_office": return <Badge variant="secondary">QA Office</Badge>
...
export default function UserProfile() {
  ...
  const handleAvatarChange = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (file) {
        if (file.size > 2 * 1024 * 1024) { showToast('error', 'Profile picture must be 2MB or smaller.'); return }
        try {
          const dataUrl = await readFileAsDataUrl(file)
          setAvatar(dataUrl)
          showToast('success', 'Avatar updated')
        } catch { showToast('error', 'Failed to update avatar') }
      }
    }
    input.click()
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader
        ...
        actions={
          isEditing ? (
            <>
              <Button variant="outline" onClick={() => { setIsEditing(false); if (user) { setName(user.name); setDepartment(user.department) } }} className="h-9">Cancel</Button>
              <Button className="h-9 shadow-sm" onClick={handleSave}><Save className="w-4 h-4 mr-2" />Save Changes</Button>
            </>
          ) : (
            <Button onClick={() => setIsEditing(true)} className="h-9">Edit Profile</Button>
          )
        }
      />
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative">
                  <Avatar className="h-20 w-20">
                    ...
                    <AvatarFallback className="text-lg bg-gray-100 text-gray-600">{user?.name ? getInitials(user.name) : "U"}</AvatarFallback>
                  </Avatar>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input id="name" value={name} onChange={(e) => setName(e.target.value)} disabled={!isEditing} className="h-10" />
                </div>
                <div className="grid gap-2">
                  <Label className="text-[13px] font-medium">Email Address</Label>
                  <Input id="email" value={user?.email ?? ""} disabled className="h-10 bg-gray-50" />
                </div>
                <div className="grid gap-2">
                  <Label className="text-[13px] font-medium">Department</Label>
                  <Input id="department" value={department} onChange={(e) => setDepartment(e.target.value)} disabled={!isEditing} className="h-10" />
                </div>
                <div className="grid gap-2">
                  <Label className="text-[13px] font-medium">Role</Label>
                  <Input id="role" value={roleLabel} disabled className="h-10 bg-gray-50" />
                </div>
                <div className="grid gap-2">
                  <Label className="text-[13px] font-medium">Member Since</Label>
                  <Input id="member-since" value={user?.memberSince || ""} disabled className="h-10 bg-gray-50" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200/60 shadow-sm">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50/50">
                ...
                <span className="text-[14px] font-semibold text-gray-900">{docCount}</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50/50">
                <span className="text-[13px] text-gray-500">Requests Made</span>
                <span className="text-[14px] font-semibold text-gray-900">{requestCount}</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50/50">
                <span className="text-[13px] text-gray-500">Approved</span>
                <span className="text-[14px] font-semibold text-emerald-600">{approvedCount}</span>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      {toastMessage && (
        <div className={`fixed bottom-4 right-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg border ${toastMessage.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
          {toastMessage.type === 'success' ? <CheckCircleIcon className="w-5 h-5 text-emerald-600" /> : <div className="w-5 h-5 text-red-600 flex items-center justify-center">!</div>}
