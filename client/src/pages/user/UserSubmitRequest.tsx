...
import { getOnlineDocument } from "@/services/documents"
...
export default function UserSubmitRequest({ docIds = [], onBack, onSuccess }: UserSubmitRequestProps) {
  ...
  useEffect(() => {
    async function loadDocs() {
      setIsLoading(true)
      const docs = await Promise.all(docIds.map((id) => getOnlineDocument(id)))
      setSelectedDocs(docs.filter(Boolean) as Document[])
      setIsLoading(false)
    }
    if (docIds.length > 0) loadDocs()
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createRequest({
        title: `Document Request - ${new Date().toLocaleDateString()}`,
        purpose,
        remarks,
        priority,
        documents: selectedDocs.map((d) => ({ documentId: d.id, documentName: d.name })),
      })
      setShowSuccess(true)
      setTimeout(() => {
        onSuccess?.()
  ...
  const getFileIcon = () => <FileText className="w-4 h-4 text-gray-400" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl">
        <Card className="border-gray-200/60 shadow-sm">
          ...
          <CardContent className="space-y-3">
            {isLoading ? (
              <p className="text-[13px] text-gray-500 py-4 text-center">Loading documents...</p>
            ) : selectedDocs.length === 0 ? (
              <p className="text-[13px] text-gray-500 py-4 text-center">No documents selected</p>
            ) : (
