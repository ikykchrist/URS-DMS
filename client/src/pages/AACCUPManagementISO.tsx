...
interface AACCUPManagementISOProps {
...
export default function AACCUPManagementISO({ sidebarCollapsed: _sidebarCollapsed = false }: AACCUPManagementISOProps) {
  ...
  const fetchData = async () => {
    const [areasData, submissionsData] = await Promise.all([
      listAllOnlineAaccupAreas({ areaSet: "ISO" }),
      listAllOnlineSubmissions({ areaSet: "ISO" })
    ])
    setAreas(areasData)
  ...
  const handleExport = () => {
    ...
    link.href = url
    link.download = `iso-areas-${new Date().toISOString().slice(0, 10)}.csv`
    link.click()
    ...
    <>
      <div className="p-4 sm:p-6 lg:p-8">
        <PageHeader
          ...
          description="Manage ISO 21001 areas, submissions, and compliance tracking"
          actions={
