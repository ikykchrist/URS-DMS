...
| 8.5 | **Background Jobs, Concurrency & Heavy-Load Reliability** — Redis + BullMQ infrastructure (`lib/redis.ts`, `lib/queue.ts`, `ioredis` + `bullmq`); 4 worker types (`workers/` — folderCopy, folderZip, email, maintenance) with exponential backoff + idempotency; folder copy jobs migrated from in-process IIFE to BullMQ; ZIP generation enqueued as background job storing temp MinIO artifact with Redis TTL expiration; email delivery migrated from `setInterval` poller to BullMQ; maintenance jobs enqueued via BullMQ; graceful shutdown with Redis/BullMQ/Prisma disconnect sequence + 10s force-kill; health endpoint extended with Redis status, 4 queue depth metrics, memory RSS; Prisma connection pool config (`connection_limit=20`); env vars (`REDIS_HOST/PORT`, `WORKER_CONCURRENCY`, `JOB_RETRY_LIMIT`, `JOB_TIMEOUT_MS`, `ZIP_EXPIRATION_SECONDS`); `scripts/load-test.ps1` (5/25 concurrent users); repository regression 49/49; maintenance regression passed | ✅ done |
